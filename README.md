...
요청 이벤트를 받으면 검색 worker가 대기 중이더라도 트리거된 issue에 접수 ACK와 Actions 실행 링크를 먼저 남깁니다. 같은 요청 실행을 재시도해도 ACK 댓글은 중복 생성하지 않습니다.
...
Issue worker는 `asyncio`로 Scrapy subprocess와 progress 파일을 감시하고, `aiohttp`로 GitHub 결과 댓글을 비동기 갱신합니다. 파일 대기나 HTTP 요청 때문에 worker 이벤트 루프가 동기식으로 멈추지 않습니다.
