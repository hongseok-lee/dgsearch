...
또한 5분마다 open issue를 확인합니다. 가장 오래된 open issue를 매번 검색한 뒤 결과를 댓글로 남기고 닫습니다. 검색어는 신뢰된 사용자의 최신 댓글을 우선 사용하며, 댓글이 없으면 issue 본문을 사용합니다. GitHub Actions 예약 실행의 최소 간격이 5분이므로 1분 주기는 사용할 수 없습니다.

저장소 소유자·멤버·협업자가 open issue에 새 댓글을 달면 예약 실행을 기다리지 않고 댓글 본문을 검색어로 즉시 처리합니다. GitHub Actions 봇이 작성한 결과 댓글과 외부 사용자의 댓글은 검색어로 사용하지 않습니다.
